...
class Trainer:
    def __init__(self, df, original_data_path, augmented_save_path, model_name, epochs, batch_size, lr, drop_out,
    ...
    def _add_strata_to_df(self):
        print("Generating strata info for the original dataframe...")
        widths, heights = [], []
    ...
    def run(self):
        self._add_strata_to_df() # K-Fold 분할 전에 strata 생성
        ...
        if self.k_fold:
            ...
            for fold, (train_idx_safe, val_idx_safe) in enumerate(skf.split(df_safe, df_safe["strata"])):
                ...
                final_train_df = pd.concat([
                    fold_train_df_orig, 
                    augmented_info_df[~augmented_info_df['ID'].isin(fold_train_df_orig['ID'])]
    ...
    def train_fold(self, fold, train_df, val_df, train_data_path):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            if val_f1 > best_f1:
                ...
                torch.save(model.state_dict(), os.path.join(fold_save_dir, model_save_name))
            else:
                trigger += 1
...
#             for fold, (train_idx_safe, val_idx_safe) in enumerate(skf.split(df_safe, df_safe["strata"])):
#                 train_df = pd.concat([
#                     df_safe.iloc[train_idx_safe],
#                     df_problem
#                 ]).reset_index(drop=True)
#                 val_df = df_safe.iloc[val_idx_safe].reset_index(drop=True)
...
#             print(f"\n=== Holdout(Aspect-aware): Train={len(train_df)}, Val={len(val_df)} ===")
...
#         model = timm.create_model(self.model_name, pretrained=True, num_classes=self.num_classes, drop_path_rate=self.drop_out).to(self.device)
#         loss_fn = nn.CrossEntropyLoss()
#         optimizer = torch.optim.AdamW(model.parameters(), lr=self.lr, weight_decay=self.weight_decay) # [수정]
