...
def get_val_tta_transforms(img_size):
    ...
    - SmallestMaxSize: 이미지 비율을 유지하면서 작은 변을 기준으로 리사이즈
    - CenterCrop: 중앙 기준으로 정사각형 크롭하여 데이터 손실 최소화
    """
    return [
        # 원본
        A.Compose([
...
pred_df.to_csv(f"{output_path}/pred_advanced_tta2_efnv2xl_resize2.csv", index=False)

log.info(f"\n✅ Prediction completed and saved to {output_path}/pred_advanced_tta2_efnv2xl_resize2.csv")
